    ...
    @if (session('success'))
        <div class="mx-auto my-4 w-xl">
            <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg">
                <p class="text-lg font-semibold">Edit User: Successful</p>
                <p>{{ session('success') }}</p>
            </div>
        </div>
    @endif
    ...
    @if (session('error'))
        <div class="mx-auto my-4 w-xl">
            <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg">
                <p class="text-lg font-semibold">Edit User: Failed</p>
                <p>{{ session('error') }}</p>
            </div>
        </div>
    @endif
            ...
            <div class="card-body p-2 mx-2">
                ...
                <div class="flex flex-col mt-2">
                    ...
                    <input class="p-2 w-full border rounded border-gray-200" name="username" type="text"
                        placeholder="Enter username here..." autocomplete="off" value="{{ $user->username }}">
                </div>
                <!-- email -->
                <div class="flex flex-col mt-2">
                    ...
                    <input class="p-2 w-full border rounded border-gray-200" name="email" type="text"
                        placeholder="Enter email here..." autocomplete="off" value="{{ $user->email }}">
                </div>
                <!-- full name -->
                <div class="flex flex-col mt-2">
                    ...
                    <input class="p-2 w-full border rounded border-gray-200" name="fullname" type="text"
                        placeholder="Enter fullname here..." autocomplete="off" value="{{ $user->name }}">
                </div>
            </div>
